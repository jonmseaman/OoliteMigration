...
# test_link_libs <test.cpp>: the system libraries one test links on top of link_libs, from its
# "// oofnd-link-windows:" line (Windows only; e.g. test_http: WinHTTP and winsock). Mirrors
...
build_one() {
	...
	local -a libs
	name="$(basename "$t" .cpp)"
	read -r -a libs <<< "$(test_link_libs "$t")"
	if [ "$kind" = plain ]; then
		out="$work/$name"
		"${cc[@]}" "${flags[@]}" "${incs[@]}" -c "$t" -o "$out.o" > "$out.build.log" 2>&1 			&& "$CXX" "${flags[@]}" "$out.o" "${link_libs[@]}" "${libs[@]}" -o "$out.exe" >> "$out.build.log" 2>&1
	else
		out="$work/${name}_asan"
		"${cc[@]}" "${asan_flags[@]}" "${incs[@]}" -c "$t" -o "$out.o" > "$out.build.log" 2>&1 			&& "$CXX" "${asan_flags[@]}" -fuse-ld=lld "$out.o" "${link_libs[@]}" "${libs[@]}" -o "$out.exe" >> "$out.build.log" 2>&1
	fi
}
