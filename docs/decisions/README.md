...
| [0042](0042-oofnd-log-file-writer.md) | The Latest.log writer is `oo::log::FileWriter` (oofnd), byte for byte as OOAsyncLogger (CRLF via GNUstep's non-literal split, lone-surrogate lines dropped, 1 GiB saturation, postamble); the last real NSLogs become `OO_LOG("unclassified")` (setup-failure messages change format on stderr only); the gnustep NSLog capture stays until oo-qps | Proposed — default in effect |
| [0043](0043-oofnd-http-client.md) | The OXZ manager downloads through `oo::http::Download` (oofnd): WinHTTP on Windows (an OS component: UCRT64 has no libcurl, none added), libcurl at Phase 5; callbacks drained on the frame loop where the run loop delivered them; `NSFileHandle` becomes `FILE *`; unfetchable URLs fail at once where NSURLConnection never called back | Proposed — default in effect |
