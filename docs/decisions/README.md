...
| [0038](0038-deferred-calls-on-the-frame-loop.md) | `performSelector:withObject:afterDelay:` becomes `OOScheduleDeferredCall`, a steady-clock queue fired from the frame loop in the run loop's measured order (first due in scheduling order, two per pass), `NSTimer`'s exception handling kept | Proposed � default in effect |
| [0041](0041-debug-console-on-a-plain-socket.md) | The debug console client is one non-blocking socket reproducing the Foundation streams' measured statuses, errors and log lines; the frame loop waits on it (select, ms rounded up) while it is open | Proposed � default in effect |
