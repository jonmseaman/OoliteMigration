...
/*	DESIGN NOTES
	...
	just after the OOOpenGLState type (C++ has no forward declaration of a
	static const object).
*/
...
static const OOOpenGLState kStandardStates[OPENGL_STATE_INTERNAL_USE_ONLY + 1] =
{
	[OPENGL_STATE_INTERNAL_USE_ONLY] =
	{
		...
		.LIGHT6					= kStateFalse,
		.LIGHT7					= kStateFalse,
		.TEXTURE_2D				= kStateFalse,
		.COLOR_MATERIAL			= kStateFalse,
		...
		.NORMALIZE				= kStateFalse,
		.RESCALE_NORMAL			= kStateFalse,
		.DEPTH_TEST				= kStateFalse,
		.DEPTH_WRITEMASK		= true,
		.CULL_FACE				= kStateFalse,
	...
	[OPENGL_STATE_OPAQUE] =
	{
		...
		.NORMALIZE				= kStateFalse,
		.RESCALE_NORMAL			= kStateFalse,
		.DEPTH_TEST				= kStateTrue,
		.DEPTH_WRITEMASK		= true,
		.CULL_FACE				= kStateTrue,
		.CULL_FACE_MODE			= GL_BACK,
		.FRONT_FACE				= GL_CCW,
	},
...
static void GetCurrentOpenGLState(OOOpenGLState *state)
{
	...
	#define ITEM_STATEFLAG(NAME)		OOGL(state->NAME = (StateFlag)glIsEnabled(GL_##NAME))
