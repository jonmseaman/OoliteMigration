...
When the session's model is the frontier model and it should take the Phase 0 seams as well, run
`git config beads-worker.labels "fleet frontier"` in the checkout and paste this instead (the
scripts read the git config whenever `BEADS_WORKER_LABELS` is unset; the contract names both labels):
...
(2026-09-17, phase 0). Do not put `BEADS_WORKER_LABELS='...'` inside the wrapper: the labels come
from `git config beads-worker.labels`, and a quoted assignment inside the `-lc` string is one
escaping layer too many when the command reaches `/goal gate add` through another program (the
stored gate then carries `\'` and fails on every attempt):

```
/goal gate add C:\Users\jon\scoop\apps\msys2\current\usr\bin\bash.exe -lc "cd /c/Users/jon/OoliteMigration && MSYSTEM=UCRT64 .agents/skills/beads-worker/scripts/goal-gate.sh 3"
```
